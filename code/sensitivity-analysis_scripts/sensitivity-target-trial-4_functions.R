...
tt_4_regimes_infinite_sensitivity_bs_function <- function(dat=sensitivity_trial_4$basedata, x, natural=FALSE){  # for bootstrap (without printing)
  dat <- dat %>% mutate(assigned=pmax(baseline_volume+x, 0)) %>% 
    group_by(op_npi.new) %>% 
    mutate(w_t_x = case_when(surgeon_period==1 ~ 1, # PERIOD 1
                             surgeon_period==2 & volume_lag1==assigned ~ 1, # PERIOD 2
...
tt_4_nonparallel_sensitivity_function <- function(range){
  if(!require("pacman",character.only=T)){install.packages("pacman")}
  pacman::p_load(parallel, ParallelLogger, dplyr, pbapply, lubridate, here)
  t1 <- Sys.time()
  output <- pblapply(setNames(range, range), function(assign) {tt_4_regimes_infinite_sensitivity_function(x=assign) %>% tt_4_outcome_sensitivity_function(dat=.) %>% .$estimate} ) %>% unlist() %>% {data.frame("assign"=range, "preds"=.)}
  t2 <- Sys.time()
  print(t2-t1)
  return(output)
}

tt_4_parametric_nonparallel_sensitivity_function <- function(range){
  if(!require("pacman",character.only=T)){install.packages("pacman")}
  pacman::p_load(parallel, ParallelLogger, dplyr, pbapply, lubridate, here)
  t1 <- Sys.time()
  output <- pblapply(setNames(range, range), function(assign) {tt_4_regimes_infinite_sensitivity_function(x=assign) %>% mutate(x=assign)}
  ) %>% bind_rows(.) %>% tt_4_outcome_parametric_sensitivity_function(dat=., range=range)
  t2 <- Sys.time()
  print(t2-t1)
  return(output)
