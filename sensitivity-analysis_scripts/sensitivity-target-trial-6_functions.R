# sensitivity analysis trial 6
tt_6_regimes_finite_sensitivity_function <- function(dat=sensitivity_trial_4$basedata, x, natural=FALSE){ 
...
tt_6_parallel_sensitivity_function <- function(cores, range){
  ...
  clusterExport(cluster, c("sensitivity_trial_4", "tt_6_regimes_finite_sensitivity_function", "tt_4_outcome_sensitivity_function", "dynamic_assignment_function"))
  output <- parLapply(cl=cluster,
                      setNames(range, range), function(assign) {tt_6_regimes_finite_sensitivity_function(x=assign) %>% tt_4_outcome_sensitivity_function(dat=.) %>% .$estimate} ) %>% unlist() %>% {data.frame("assign"=range, "preds"=.)}
  t2 <- Sys.time()
  print(t2-t1)
...
tt_6_parametric_parallel_sensitivity_function <- function(cores, range){
  if(!require("pacman",character.only=T)){install.packages("pacman")}
  pacman::p_load(parallel, ParallelLogger, dplyr, pbapply, lubridate, here)
  t1 <- Sys.time()
  cluster <- makeCluster(numberOfThreads = cores)
  clusterEvalQ(cluster, {library(dplyr); library(magrittr); library(lubridate); library(splines)})
  clusterExport(cluster, c("sensitivity_trial_4", "tt_6_regimes_finite_sensitivity_function", "tt_4_outcome_parametric_sensitivity_function", "dynamic_assignment_function"))
  output <- parLapply(cl=cluster,
                      setNames(range, range), function(assign) {tt_6_regimes_finite_sensitivity_function(x=assign) %>% mutate(x=assign)}
